...
void motor::state::GameState::update (const motor::StateManager * st)
{
	...
	if(true)
	{
		...
		if(player.velocity.y >= 0)
		{
			...
			for(int x = (((int)prevPlayerHitbox.x) / 16 - 1) * 16; x <= ((int)(currPlayerHitbox.x + currPlayerHitbox.width) / 16 + 1) * 16;
					x += 16)
			{
				for(int y = (((int)prevPlayerHitbox.y) / 16 + 3) * 16; y <= ((int)(currPlayerHitbox.y + currPlayerHitbox.height) / 16 + 1) * 16;
						y += 16)
				{
		...
		{
			Rectangle b1 = Rectangle((int)(phb.x) / 16 * 16, (int)(phb.y + phb.height) / 16 * 16, 16, 16);
			Rectangle b2 = Rectangle((int)(phb.x + phb.width) / 16 * 16, (int)(phb.y + phb.height) / 16 * 16, 16, 16);
			Rectangle b3 = Rectangle((int)(phb.x + phb.width / 2) / 16 * 16, (int)(phb.y + phb.height) / 16 * 16, 16, 16);
			if(
					level.get(Vector2(b1.x / 16, b1.y / 16)) != 0 || 
					level.get(Vector2(b2.x / 16, b2.y / 16)) != 0 ||
					level.get(Vector2(b3.x / 16, b3.y / 16)) != 0)
			{
			}
			else
			{
		...
		//update the hitboxes to get the new y coordinate
		prevPlayerHitbox = prev;
		prevPlayerHitbox.width *= playerScale;
		prevPlayerHitbox.x += 7.0*playerScale;;
		//prevPlayerHitbox.width -= 7.0*playerScale + 6.0*playerScale;
		...
		{
			for(int x = (((int)(prevPlayerHitbox.x + prevPlayerHitbox.width)) / 16 - 1) * 16; x <= ((int)(currPlayerHitbox.x + currPlayerHitbox.width) / 16 + 1) * 16;
					x += 16)
			{
		...
		{
			for(int x = (((int)(prevPlayerHitbox.x + prevPlayerHitbox.width)) / 16) * 16; x >= ((int)(currPlayerHitbox.x) / 16 - 1) * 16;
					x -= 16)
			{
				for(int y = (((int)prevPlayerHitbox.y) / 16) * 16; y <= ((int)(currPlayerHitbox.y + currPlayerHitbox.height) / 16 - 1) * 16;
						y += 16)
				{
					...
					Rectangle object = Rectangle(x,y,16,16);
					if(phb.collides(object) || 
							object.intersectslineRight(Line(Vector2(prevPlayerHitbox.x, prevPlayerHitbox.y-0.01),Vector2(currPlayerHitbox.x, currPlayerHitbox.y-0.01))))
					{
						cout << "colliding left\n";
						player.velocity.x = 0;
						player.acceleration.x = 0;
